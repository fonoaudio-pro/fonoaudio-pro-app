...
const VoiceAnalysisPanel: React.FC<VoiceAnalysisPanelProps> = ({ patientId }) => {
        ...
        try {
            ...
            if (facts) {
                ...
                const nbaResults = await nbaService.getSuggestions(
                    ...
                    [{ moduleId: 'voice', data: result }]
                );
                
                const voiceSuggestions = nbaResults.results.find(r => r.moduleId === 'voice')?.actions || [];
                setSuggestions(voiceSuggestions);
            }
        } catch (error) {
            console.error('Error analyzing voice:', error);
            addToast({ message: 'Error al analizar voz', type: 'error' });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleResolve = async (factIds: string[]) => {
        try {
            await Promise.all(factIds.map(id => voiceService.resolveFact(id, 'current-user-id')));
    ...
    const handleNBAAction = async (action: NextBestAction) => {
        try {
            await nbaService.recordDecision(patientId, {
                ...
                metadata: { ...action.metadata, moduleId: 'voice' }
            });
    ...
    const handleEditNBAAction = async (modifiedAction: NextBestAction) => {
        try {
            await nbaService.recordDecision(patientId, {
                ...
                disposition: 'edited',
                metadata: { ...modifiedAction.metadata, moduleId: 'voice' }
            });
    ...
    return (
        <div className="space-y-4">
            ...
            <BaseAnalysisPanel 
                title="Análisis de Función Vocal"
                analysis={analysis}
                isLoading={isLoading}
                onResolve={handleResolve}
                riskColors={RISK_COLORS}
            />
            <AnalysisHistoryTimeline patientId={patientId} module="voice" />
