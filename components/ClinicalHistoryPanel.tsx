...
import { DocumentScannerResult } from './DocumentScanner';
...
export default function ClinicalHistoryPanel({
  ...
  const [ocrAutoFillPending, setOcrAutoFillPending] = useState(false);
  ...
  /**
   * Auto-fill clinical history fields from OCR extracted data.
   * Maps common ORL document fields to template fields.
   */
  function handleOCRDataReady(result: DocumentScannerResult) {
    if (!selectedTemplate) return;
    setOcrAutoFillPending(true);
    ...
    for (const section of selectedTemplate.schema_json) {
      for (const field of section.fields) {
        const key = `${section.section_id}__${field.id}`;
        if (newValues[key]) continue; // Don't overwrite existing values

        const lf = (field.label || '').toLowerCase();
        const sf = (section.section_id || '').toLowerCase();

        // Medical data (ExtractedMedicalData)
        if ('diagnosis' in data && data.diagnosis) {
          if (lf.includes('diagnóstico') || lf.includes('diagnostico') || lf.includes('impresión')) {
            newValues[key] = data.diagnosis;
            continue;
          }
        }
        if ('medications' in data && data.medications?.length) {
          if (lf.includes('medicación') || lf.includes('medicacion') || lf.includes('tratamiento') || lf.includes('fármaco')) {
            newValues[key] = data.medications.join(', ');
            continue;
          }
        }
        if ('observations' in data && data.observations) {
          if (lf.includes('observación') || lf.includes('observacion') || lf.includes('hallazgo')) {
            newValues[key] = data.observations;
            continue;
          }
        }
        if ('testResults' in data && data.testResults?.length) {
          if (lf.includes('resultado') || lf.includes('prueba') || lf.includes('test') || lf.includes('escala')) {
            const testSummary = data.testResults.map(t => `${t.testName}: ${t.value}`).join('\n');
            newValues[key] = testSummary;
            continue;
          }
        }

        // Anamnesis data (AnamnesisData)
        if ('motivoConsulta' in data && data.motivoConsulta) {
          if (lf.includes('motivo') && (sf.includes('motivo') || sf.includes('consult'))) {
            newValues[key] = data.motivoConsulta.motivoPrincipal || '';
            continue;
          }
          if (lf.includes('cronolog') || lf.includes('inicio') || lf.includes('evolución')) {
            newValues[key] = data.motivoConsulta.cronologia || '';
            continue;
          }
          if (lf.includes('tratamiento previo') || lf.includes('tratamientos')) {
            newValues[key] = data.motivoConsulta.tratamientosPrevios || '';
            continue;
          }
        }
        if ('antecedentesPersonales' in data && data.antecedentesPersonales) {
          if (lf.includes('antecedente') && (lf.includes('person') || lf.includes('médico') || lf.includes('patolog'))) {
            newValues[key] = data.antecedentesPersonales.historiaMedica || '';
            continue;
          }
          if (lf.includes('otolog') || lf.includes('auditivo') || lf.includes('oído') || lf.includes('oido')) {
            newValues[key] = data.antecedentesPersonales.antecedentesOtologicos || '';
            continue;
          }
        }
        if ('antecedentesFamiliares' in data && data.antecedentesFamiliares) {
          if (lf.includes('familiar') || lf.includes('dinámica') || lf.includes('entorno')) {
            newValues[key] = data.antecedentesFamiliares.historiaFamiliar || '';
            continue;
          }
        }
        if ('areasEspecificas' in data && data.areasEspecificas) {
          if (lf.includes('voz') && !lf.includes('antecedente')) {
            newValues[key] = data.areasEspecificas.voz || '';
            continue;
          }
          if (lf.includes('lenguaje') || lf.includes('comunicación') || lf.includes('comunicacion')) {
            newValues[key] = data.areasEspecificas.lenguajeComunicacion || '';
            continue;
          }
          if (lf.includes('habla') || lf.includes('articulación') || lf.includes('articulacion')) {
            newValues[key] = data.areasEspecificas.habla || '';
            continue;
          }
          if (lf.includes('audición') || lf.includes('audicion') || lf.includes('auditiv')) {
            newValues[key] = data.areasEspecificas.audicion || '';
            continue;
          }
          if (lf.includes('deglución') || lf.includes('deglucion') || lf.includes('tragar')) {
            newValues[key] = data.areasEspecificas.deglucion || '';
            continue;
          }
        }

        // Fallback: if field is empty and OCR has raw text, put relevant snippet
        if (lf.includes('texto del informe') || lf.includes('contenido') || lf.includes('extracto')) {
          if (ocrText.length > 0) {
            newValues[key] = ocrText.slice(0, 2000);
          }
        }
      }
    }
