import { CognitionSign, CognitionMeasure, CognitionRule, RiskLevel, ActionLevel, CognitionAnalysisResult } from './types';
import { COGNITION_RULES } from './rules/cognition_rules';
import { COGNITION_MAPPING } from './mapping/cognition_mapping';
import { ClinicalFact, RedFlag, Observation } from '../../types';
import { supabase } from '../../../utils/supabaseClient';

export class CognitionService {
  async analyze(facts: ClinicalFact[], patientId?: string): Promise<CognitionAnalysisResult> {
    const detectedSigns: CognitionSign[] = [];
    const detectedMeasures: CognitionMeasure[] = [];
    
    const allSigns = Object.keys(COGNITION_MAPPING) as CognitionSign[];
    const allMeasures: CognitionMeasure[] = [
      'mmse_score',
      'moca_score',
      'clock_drawing_score',
    ...
    facts.forEach(fact => {
      if (!fact.isResolved && fact.category === 'cognition') {
        const signValue = fact.sign;
        if (allSigns.includes(signValue as CognitionSign)) {
          detectedSigns.push(signValue as CognitionSign);
        } else if (allMeasures.includes(signValue as CognitionMeasure)) {
          detectedMeasures.push(signValue as CognitionMeasure);
        }
      }
    });
    ...
    const needsInstrumentalAssessment = maxSeverity === 'high' || maxSeverity === 'critical' || detectedSigns.includes('anosognosia');

    const professionalSummary = detectedSigns
      .map(sign => COGNITION_MAPPING[sign].professional.title)
      .join(', ') || 'Sin signos detectados';

    const familySummary = detectedSigns
      .map(sign => COGNITION_MAPPING[sign].family.title)
      .join(', ') || 'No se detectaron señales de alerta';

    const result: CognitionAnalysisResult = {
  ...
  async saveAnalysisSnapshot(patientId: string, result: CognitionAnalysisResult): Promise<void> {
